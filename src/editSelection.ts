...
async function getAIEditForSelection(query: string, selectedText: string, settings: AISettings): Promise<string> {
    ...
    if (settings.provider === 'groq') {
        ...
        const result = await ollamaService.generateContent(settings.model, messages as any);
        const trimmedResult = result.trim();
        if (trimmedResult && trimmedResult.length > 0) {
            return trimmedResult;
        }
        ...
        const result = await openRouterService.generateContent(settings.model, messages as any);
        const trimmedResult = result.trim();
        if (trimmedResult && trimmedResult.length > 0) {
            return trimmedResult;
        }
        ...
        const result = await nvidiaService.generateContent(settings.model, messages as any);
        const trimmedResult = result.trim();
        if (trimmedResult && trimmedResult.length > 0) {
            return trimmedResult;
        }
